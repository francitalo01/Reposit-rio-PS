...
$\begin{cases}
    H_{0}: \mbox{A distribuição dos dados de IMC para cada esporte é normal.} \\
    H_{1}: \mbox{A distribuição dos dados de IMC para cada esporte não é normal.} \\
  \end{cases}
$\\
...
```
$\begin{cases}
    H_{0}: \mbox{Não há diferença significativa nas distribuições de IMC entre as modalidades esportivas; ou seja, os valores de IMC têm a mesma mediana entre os grupos.} \\
    H_{1}: \mbox{Há pelo menos uma diferença significativa nas distribuições de IMC entre as modalidades esportivas; ou seja, os valores de IMC diferem entre os grupos.} \\
  \end{cases}
$\\
...
$\begin{cases}
    H_{0}: \mbox{Não há relação entre o medalhista e o tipo de medalha conquistada. Em outras palavras, a quantidade de medalhas de ouro, prata e bronze é distribuída de forma independente entre os três atletas analisados.} \\
    H_{1}: \mbox{Existe uma relação entre o medalhista e o tipo de medalha conquistada. Isso significa que a distribuição das medalhas de ouro, prata e bronze varia significativamente entre os três atletas.} \\
  \end{cases}
$\\
...
ggplot(olimpiadas_Alt_Pes) +
  ...
  labs(
    title = "Relação entre Peso e Altura dos Atletas",
    x = "Altura (cm)",
    y = "Peso (kg)"
  ) +
  theme_estat()
...
```
$\begin{cases}
    H_{0}: \mbox{Os dados vêm de uma distribuição normal} \\
    H_{1}: \mbox{Os dados não vêm de uma distribuição normal} \\
  \end{cases}
$\\
...
bartlett_result <- bartlett.test(Altura ~ Peso_Bin, data = olimpiadas_Alt_Pes)

bartlett_df <- data.frame(Statistic = bartlett_result$statistic, P.Value = bartlett_result$p.value)

knitr::kable(bartlett_df, caption = "Teste de Bartlett para homogeneidade de variâncias entre grupos de peso", col.names = c("Estatística", "P-valor"))

```
$\begin{cases}
    H_{0}: \mbox{As variâncias dos grupos são iguais} \\
    H_{1}: \mbox{Pelo menos uma das variâncias é diferente} \\
  \end{cases}
$\\
...
levene_result <- leveneTest(Altura ~ Peso_Bin, data = olimpiadas_Alt_Pes)

levene_df <- as.data.frame(levene_result)

knitr::kable(levene_df, caption = "Teste de Levene para homogeneidade de variâncias entre grupos de peso")
