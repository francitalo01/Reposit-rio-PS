...
Para realização das analises utilizou-se um banco de dados fornecidos pelo cliente com dados que exite uma probabilidade de serem confiáveis para os Jogos Olimpíadas de 2000 a 2016 separado por paises sede da olimpiada e ano de sua realização e contém as seguintes variáveis: nome do atleta, sexo, idade, altura, peso, país que o atleta compete, esporte, evento e tipo de medalha. 
...
Na análise, busca-se identificar os países que tiveram o maior número de mulheres medalhistas nos Jogos Olímpicos de 2000 a 2016. Considerou-se as variáveis de sexo (feminino), país de origem das atletas (Time), e a presença de uma medalha (Medalha) e utilizou-se os dados das olimpiadas do periodo de 2000 a 2016. As três variáveis estudadas são variáveis qualitativas nominais. A variável sexo pode se categorizada como Masculino e Feminino, já a variável Time asume os nomes dos paises de origem dos atletas e a variável medalha pode ser Ouro, Prata e Bonze. O objetivo principal foi compreender quais nações se destacaram em termos de conquistas femininas, sem duplicações de resultados, ou seja, apenas o número de mulheres medalhistas únicas. Utilizando como métricas o número total de medalhistas por país e a frequência relativa de cada país em relação ao total de mulheres medalhistas
...
olimpiadas_IMC$Esporte <- reorder(olimpiadas_IMC$Esporte, olimpiadas_IMC$IMC, FUN = mean, decreasing = TRUE)
...
```{r}
print_quadro <- function(data, var_name, group_var, title="Medidas resumo da(o) [nome da variável]", label="quad:quadro_resumo1") {
  var_name <- substitute(var_name)
  ...
  print(knitr::kable(data, caption = title, col.names = c("Esporte", "Média", "Desvio Padrão", "Variância", "Mínimo", "1º Quartil", "Mediana", "3º Quartil", "Máximo")))
}

#print_quadro(olimpiadas_IMC, "IMC", "Esporte", title = "Medidas resumo de IMC por Esporte", label = "quad:quadro_resumo_imc_esporte")
...
\begin{quadro}[H]
	...
	\caption{Medidas resumo de IMC por Esporte}
	\centering
	\begin{tabular} { | l | S[table-format=3.2] | S[table-format=3.2] | S[table-format=3.2] | S[table-format=3.2] | S[table-format=3.2] | }
	\hline
	\textbf{Estatística} & \textbf{Judô} & \textbf{Atletismo} & \textbf{Badminton} & \textbf{Futebol} & \textbf{Ginástica} \\
	\hline
	...
	\hline
	\end{tabular}
	\label{quad:quadro_resumo_imc_esporte_invertido}
\end{quadro}
...
\begin{quadro}[H]
\centering
\caption{Teste de normalidade de Shapiro-Wilk}
...
\begin{quadro}[H]
\centering
\caption{Teste de comparação de Kruskal-Wallis}
...
```{r bar_plot,fig.cap="Gráfico de barras da distribuição das medalhas", fig.align="center"}
...
\begin{quadro}[H]
\centering
\caption{Quadro dos Medalhistas e Distribuição das Medalhas}
\setlength{\tabcolsep}{9pt}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|c|c|c|c|c|}
    \hline
    \textbf{Nome} & \textbf{Total de Medalhas} & \textbf{Medalha} & \textbf{Quantidade} \\
    \hline
    \multirow{3}{*}{Michael Fred Phelps} & \multirow{3}{*}{28} & Ouro & 23 \\
    & & Prata & 3 \\
    & & Bronze & 2 \\
    \hline
    \multirow{3}{*}{Natalie Anne Coughlin} & \multirow{3}{*}{12} & Ouro & 3 \\
    & & Prata & 4 \\
    & & Bronze & 5 \\
    \hline
    \multirow{3}{*}{Ryan Steven Lochte} & \multirow{3}{*}{12} & Ouro & 6 \\
    & & Prata & 0 \\
    & & Bronze & 3 \\
    \hline
\end{tabular}
\label{quadro_medalhistas_distribuicao}
\end{quadro}
```
...
\begin{quadro}[H]
\centering
\caption{P-valor do teste Qui-Quadrado entre as variáveis Nome do Atleta e Medalha}
...
# Verificação de Normalidade com Gráficos de Distribuição
...
Antes de realizar qualquer análise de regressão, é importante verificar se os dados seguem uma distribuição normal. A verificação da normalidade é essencial para garantir que os resultados do modelo de regressão linear sejam confiáveis. O teste de normalidade pode ser realizado de várias maneiras, incluindo a inspeção visual de histogramas ou gráficos de dispersão dos resíduos, bem como por meio de testes estatísticos.
...
\begin{quadro}[H]
\centering
\caption{P-valor do teste de normalidade (Teste de Anderson-Darling) da Altura dos Atletas}
\setlength{\tabcolsep}{9pt}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|c|c|c|c|}
       \hline
...
\begin{quadro}[H]
\centering
\caption{P-valor do teste de normalidade (Teste de Anderson-Darling) do Peso dos Atletas}
\setlength{\tabcolsep}{9pt}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|c|c|c|c|}
       \hline
...
```{r fig.cap="Análise de Diagnóstico de resíduos", fig.align="center"}
...
resutado_corre <- cor(olimpiadas_Alt_Pes$Peso, olimpiadas_Alt_Pes$Altura, method = "pearson")
...
\begin{quadro}[H]
\centering
\caption{Coeficiente de Correlação de Pearson entre Peso e Altura}
\setlength{\tabcolsep}{7pt}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|c|c|}
       \hline
       \textbf{Coeficiente} & \textbf{r}  \\
       \hline
...
```{r}
modelo_simples <- lm(Peso ~ Altura, data = olimpiadas_Alt_Pes)
summary_modelo <- summary(modelo_simples)
...
\begin{quadro}[H]
\centering
\caption{P-valor da regressão linear simples para o Peso em função da Altura}
\setlength{\tabcolsep}{9pt}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|c|c|c|c|c|}
       \hline
       ...
       \hline
       Intercepto & -122,50 & 1,76 & -69,22 & \textless 0,001 \\
       Altura & 110,34 & 0,99 & 111,37 & \textless 0,001\\
       \hline
\end{tabular}
...
```{r}
# Coeficiente de determinação (R²)
resumo_modelo <- summary(modelo_simples)
#cat("Coeficiente de Determinação (R²):", resumo_modelo$r.squared, "\n")

# Coeficiente de determinação (R²) ajustado
r2_ajustado <- summary_modelo$adj.r.squared

#cat("Coeficiente de Determinação Ajustado (R² ajustado):", round(r2_ajustado, 4), "\n")
```
...
\begin{quadro}[H]
\centering
\caption{Coeficiente de determinação (R²)}
\setlength{\tabcolsep}{7pt}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|c|c|}
       \hline
...
O coeficiente de determinação $R^2$, do **Quadro 12**, foi de aproximadamente 0,63. Isso indica que aproximadamente 63% da variação na altura pode ser explicada pela variação no peso dos atletas, considerando o modelo linear ajustado. Sugerindo assim uma associação significativa entre as variáveis. No entanto, há ainda 37% da variação na altura que não é explicada por esse modelo e pode estar relacionada a outros fatores ou variáveis que não foram incluídas nesta análise.

### Teste F da Regressão Linear Simples
...
\begin{center}
\hipoteses{O modelo de regressão não é significativo, ou seja, a variável peso não tem efeito significativo sobre a altura.}{O modelo de regressão é significativo, ou seja, a variável peso tem efeito significativo sobre a altura.}
\end{center}
...
\begin{quadro}[H]
\centering
\caption{P-valor do Teste F da Regressão}
\setlength{\tabcolsep}{7pt}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|c|c|}
       \hline
